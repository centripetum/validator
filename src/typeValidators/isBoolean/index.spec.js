...
describe('typeValidators:isBoolean', () => {
  ...
  it('returns Left(NOT_A_BOOLEAN error) when value is Just([1,2,3])', () => {
    expect(isBoolean(Just([1, 2, 3]))).toEqual(
      Left({ failures: [{ errorType: NOT_A_BOOLEAN }], value: Just([1, 2, 3]) })
    )
  })
  ...
  it(`returns Left(NOT_A_BOOLEAN error) when value is Nothing`, () => {
    expect(isBoolean(Nothing)).toEqual(
      Left({ failures: [{ errorType: NOT_A_BOOLEAN }], value: Nothing })
    )
  })
})
